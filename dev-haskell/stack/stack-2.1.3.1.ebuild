...
SLOT="0/${PV}"
KEYWORDS="~amd64 ~x86"
IUSE="disable-git-info hide-dependency-versions integration-tests static supported-build"
...
RDEPEND=">=dev-haskell/aeson-1.2.4.0:=[profile?]
	...
	>=dev-haskell/http-download-0.1.0.0:=[profile?]
	...
	>=dev-haskell/mono-traversable-1.0.9.0:=[profile?]
	...
	>=dev-haskell/pantry-0.1.1.1:=[profile?]
	...
	>=dev-haskell/resource-pool-0.2.3.2:=[profile?]
	>=dev-haskell/resourcet-1.2.1:=[profile?]
	>=dev-haskell/retry-0.7.6.3:=[profile?]
	>=dev-haskell/rio-0.1.11.0:=[profile?]
	...
	>=dev-haskell/terminal-size-0.3.2.1:=[profile?]
	...
	>=dev-haskell/unix-compat-0.5.0.1:=[profile?]
	>=dev-haskell/unliftio-0.2.12:=[profile?]
	>=dev-haskell/unordered-containers-0.2.9.0:=[profile?]
	>=dev-haskell/vector-0.12.0.1:=[profile?]
	>=dev-haskell/yaml-0.10.4.0:=[profile?]
	>=dev-haskell/zip-archive-0.3.3:=[profile?]
	>=dev-haskell/zlib-0.6.2:=[profile?]
	>=dev-lang/ghc-8.4.3:=
	!disable-git-info? ( >=dev-haskell/githash-0.1.3.0:=[profile?]
				>=dev-haskell/optparse-simple-0.1.0:=[profile?] )
	integration-tests? ( >=dev-haskell/hspec-2.4.8:=[profile?]
				>=dev-haskell/optparse-generic-1.3.0:=[profile?] )
"
DEPEND="${RDEPEND}
	>=dev-haskell/cabal-2.2.0.1
	test? ( >=dev-haskell/quickcheck-2.10.1
		>=dev-haskell/raw-strings-qq-1.1
		>=dev-haskell/smallcheck-1.1.5
		!integration-tests? ( >=dev-haskell/hspec-2.4.8 ) )
"
...
src_configure() {
	haskell-cabal_src_configure \
		...
		$(cabal_flag supported-build supported-build)
}
